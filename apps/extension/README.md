...
Each collection refreshes provider pages that were already open before reading the displayed values; newly created tabs are allowed to load normally without an unnecessary second navigation. Kilo is collected from its dedicated Credits page. If it temporarily reports a suspicious zero balance, the extension preserves the prior verified balance and retries only Kilo after 30 seconds, without refreshing the other provider pages. The popup also includes an optional **Close provider tabs opened by Capacity Monitor after collection** setting. It affects only tabs the extension created; tabs you already had open are never closed.
