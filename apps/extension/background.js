...
async function runCollection({ permitted: targets = [], missing = [] }) {
  ...
  for (const outcome of outcomes) {
    ...
    for (const definition of target.metrics) {
      ...
      const parsedReading = parsedByKey[definition.key];
      const parsed = parsedReading ? withResetMetadata(parsedReading, definition, now) : null;
...
function withResetMetadata(metric, definition, now) {
  if (!definition.resetWindowMs) return metric;
  const resetAt = resolveResetAt(metric.resetText, new Date(now));
  return { ...metric, resetWindowMs: definition.resetWindowMs, ...(resetAt ? { resetAt } : {}) };
}

function resolveResetAt(resetText, now) {
  if (!resetText) return null;
  const countdown = [...resetText.matchAll(/(\d+)\s*(day|days|d|hour|hours|hr|hrs|h|min|mins|minute|minutes|m)/gi)]
    .reduce((milliseconds, match) => milliseconds + Number(match[1]) * ({ d: 86400000, day: 86400000, days: 86400000, h: 3600000, hr: 3600000, hrs: 3600000, hour: 3600000, hours: 3600000, m: 60000, min: 60000, mins: 60000, minute: 60000, minutes: 60000 }[match[2].toLowerCase()] ?? 0), 0);
  if (countdown) return new Date(now.getTime() + countdown).toISOString();
  const dateText = resetText.replace(/^Resets\s*/i, "");
  ...
  if (reset < now && dateOnly) reset.setFullYear(reset.getFullYear() + 1);
  return reset.toISOString();
}
